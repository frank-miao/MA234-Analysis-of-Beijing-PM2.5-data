...
def cross_validation(dataset, model, model_type:str):
    ...
    for train_index, test_index in tqdm(kf.split(X)):
        ...
        _, train_score, test_score = model(data_loader)
        score_train.append(train_score)
        score_test.append(test_score)

    train_mean = sum(score_train) / len(score_train)
    test_mean = sum(score_test) / len(score_test)

    if model_type == 'classification':
        print('The average f1 score of train dataset is ' + str(train_mean), end=' ')
        print('The average f1 score of test dataset is ' + str(test_mean))

    elif model_type == 'regression':
        print('The average r2 score of train dataset is ' + str(train_mean), end=' ')
        print('The average r2 score of test dataset is ' + str(test_mean))
...
def model_call(model_name: str, dataloader, print_flag=False):
    if model_name in ['KNN', 'knn']:
        ...
        return regression_models.lstm(dataloader, print_flag)
